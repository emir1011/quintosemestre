...
class Meteor(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load("Superman_S_symbol.svg-2.png").convert()
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load("destacada-superman-2.jpg").convert()
        self.image.set_colorkey(WHITE)
        self.rect = self.image.get_rect()
...
class Game(object):
    ...
    def display_frame(self, screen):
        ...
        if self.game_over:
            ...
            text = font.render("Has pasado al segundo nivel", True, BLACK)  # TEXTO
            center_x = (SCREEN_WIDTH // 2) - (text.get_width() // 2)  # position text
